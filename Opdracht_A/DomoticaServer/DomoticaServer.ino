...
byte mac[] = { 0x2C, 0xF7, 0xF1, 0x08, 0x27, 0x02 }; // Ethernet adapter shield S. Oosterhaven
...
static constexpr uint8_t SENSOR0_PIN = A1;
static constexpr uint8_t SENSOR1_PIN = A2;
...
bool remoteSwitchStates[] = { OFF, OFF, OFF };
...
void loop()
{
   ...
   while (ethernetClient.connected()) 
   {
      ...
      // Activate pin based on pinState
      if (pinChange)
      {
        if (pinState)
        {
...
{     
         ...
         case 'i':    
            ...
            break;
         case '0':
            toggleRemoteSwitch(0);
            server.write(remoteSwitchStates[0] ? " ON\n" : "OFF\n");
            break;
         case '1':
            toggleRemoteSwitch(1);
            server.write(remoteSwitchStates[1] ? " ON\n" : "OFF\n");
            break;
         case '2':
            toggleRemoteSwitch(2);
            server.write(remoteSwitchStates[2] ? " ON\n" : "OFF\n");
            break; 
         case 'x':
            intToCharBuf(readSensor(SENSOR0_PIN, 1000), buf, 4);
            server.write(buf, 4);
            break;
         case 'y':
            intToCharBuf(readSensor(SENSOR1_PIN, 1000), buf, 4);
            server.write(buf, 4);
            break;
         default:
            digitalWrite(infoPin, LOW);
...
void toggleRemoteSwitch(const uint8_t switchId)
{
  remoteSwitchStates[switchId] = !remoteSwitchStates[switchId];
  apa3Transmitter.sendUnit(switchId, remoteSwitchStates[switchId]);
}
